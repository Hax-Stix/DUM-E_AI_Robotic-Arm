...
# FIXME: Possible Problem With Speech_Recognition

import RPi.GPIO as GPIO

# INFO:RPi.GPIO Library Not Avaliable On Anything Other Than RPI Boards
...
def Claw_Fully_Open():
    if Claw_Pos <= 9:
        if GPIO.input(Claw_Limit_Upper) == GPIO.HIGH:
            ...
            while GPIO.input(Claw_Limit_Upper) == GPIO.LOW and Claw_Pos \
                != abs(10):
                ...
                if GPIO.input(Claw_Limit_Upper) == GPIO.LOW \
                    and Claw_Pos != abs(10) \
                    or GPIO.input(Claw_Limit_Upper) == GPIO.HIGH \
                    and Claw_Pos == abs(10):
...
def Wrist_Fully_Up():
    if Wrist_Pos <= 19:
        if GPIO.input(Wrist_Limit_Upper) == GPIO.HIGH:
            ...
            while GPIO.input(Wrist_Limit_Upper) == GPIO.LOW \
                and Wrist_Pos != abs(20):
                print("DUM-Es Wrist Is Moving Up")
                GPIO.output(Wrist_DC_Motor_1, True)
                GPIO.output(Wrist_DC_Motor_2, False)
                pwm_Wrist.ChangeDutyCycle(50)
                ...
                if GPIO.input(Wrist_Limit_Upper) == GPIO.LOW \
                    and Wrist_Pos != abs(20) \
                    or GPIO.input(Wrist_Limit_Upper) == GPIO.HIGH \
                    and Wrist_Pos == abs(10):
...
def Wrist_Middle():
    if Wrist_Pos > 0:
        ...
        if GPIO.input(Wrist_Limit_Lower) \
            or GPIO.input(Wrist_Limit_Upper) == GPIO.HIGH:
            print("ERROR DUM-E Calibration NEEDED")
    ...
    if Wrist_Pos >= -19:
        if GPIO.input(Wrist_Limit_Lower) == GPIO.HIGH:
            ...
            while GPIO.input(Wrist_Limit_Upper) == GPIO.LOW \
                ...
                if GPIO.input(Wrist_Limit_Lower) == GPIO.LOW \
                    and Wrist_Pos == abs(-20) \
                    or GPIO.input(Wrist_Limit_Lower) == GPIO.HIGH \
                    and Wrist_Pos != abs(-20):
                    print("ERROR DUM-E Wrist Calibration Needed")
    else:
        print("DUM-Es Claw Is Already Located In The Middle")
